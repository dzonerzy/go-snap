...
func (b *WrapperBuilder[P]) DiscoverOnPATH(enable bool) *WrapperBuilder[P] { b.spec.DiscoverOnPATH = enable; return b }
// WorkingDir sets the working directory for the child process.
func (b *WrapperBuilder[P]) WorkingDir(dir string) *WrapperBuilder[P] { b.spec.WorkingDir = dir; return b }
// Env sets/overrides a single environment variable for the child process.
func (b *WrapperBuilder[P]) Env(key, value string) *WrapperBuilder[P] { if b.spec.Env==nil{b.spec.Env=make(map[string]string)}; b.spec.Env[key]=value; return b }
// EnvMap sets multiple environment variables.
func (b *WrapperBuilder[P]) EnvMap(vars map[string]string) *WrapperBuilder[P] { if b.spec.Env==nil{b.spec.Env=make(map[string]string)}; for k,v:= range vars { b.spec.Env[k]=v }; return b }
// InheritEnv controls whether to inherit the parent environment (default true).
func (b *WrapperBuilder[P]) InheritEnv(enable bool) *WrapperBuilder[P] { b.spec.InheritEnv = enable; return b }
// InjectArgsPre inserts arguments before forwarded args.
func (b *WrapperBuilder[P]) InjectArgsPre(args ...string) *WrapperBuilder[P] { b.spec.PreArgs = append(b.spec.PreArgs, args...); return b }
// InjectArgsPost appends arguments after forwarded args.
func (b *WrapperBuilder[P]) InjectArgsPost(args ...string) *WrapperBuilder[P] { b.spec.PostArgs = append(b.spec.PostArgs, args...); return b }
...
func (b *WrapperBuilder[P]) ForwardUnknownFlags() *WrapperBuilder[P] { b.spec.ForwardUnknown = true; return b }
// TransformArgs provides full control over the final argv.
func (b *WrapperBuilder[P]) TransformArgs(fn func(*Context, []string) ([]string, error)) *WrapperBuilder[P] { b.spec.Transform = fn; return b }
...
func (b *WrapperBuilder[P]) TransformTool(fn func(tool string, args []string) (string, []string, error)) *WrapperBuilder[P] {
...
func (b *WrapperBuilder[P]) TeeTo(out, err io.Writer) *WrapperBuilder[P] { b.spec.TeeOut = out; b.spec.TeeErr = err; return b }
...
func (b *WrapperBuilder[P]) AllowTools(names ...string) *WrapperBuilder[P] {
    ...
    b.spec.TransformToolFn = func(tool string, args []string) (string, []string, error) {
        ...
        ok := false
        for _, n := range names { if n == base { ok = true; break } }
        if !ok {
            return tool, args, NewError(ErrorTypePermission, "tool not allowed: "+base)
        }
        if prev != nil { return prev(tool, args) }
        return tool, args, nil
    }
    return b
}
// HideFromHelp hides the wrapped command from help. Only valid for command-level wrappers.
func (b *WrapperBuilder[P]) HideFromHelp() *WrapperBuilder[P] { if b.cmd!=nil { b.cmd.Hidden = true }; return b }
// Visible marks the wrapped command as visible in help.
func (b *WrapperBuilder[P]) Visible() *WrapperBuilder[P] { if b.cmd!=nil { b.cmd.Hidden = false }; return b }
...
func (b *WrapperBuilder[P]) MapBoolFlag(wrapperFlag string, childTokens ...string) *WrapperBuilder[P] {
    if b.spec.MapBool == nil { b.spec.MapBool = make(map[string][]string) }
...
func (w *WrapperSpec) run(ctx *Context, origArgs []string) error {
    // Resolve binary
    bin := w.Binary
    if bin == "" && w.Dynamic {
    ...
    if w.DiscoverOnPATH && !filepath.IsAbs(bin) {
        if p, err := exec.LookPath(bin); err == nil { bin = p }
    }
    ...
    if w.WorkingDir != "" { cmd.Dir = w.WorkingDir }
    ...
    case modePassthrough:
        ...
        if w.CaptureAlso {
            ...
            if w.TeeOut != nil { mwOut = append(mwOut, w.TeeOut) }
            mwOut = append(mwOut, &outBuf)
            outW = io.MultiWriter(mwOut...)

            mwErr := []io.Writer{errW}
            if w.TeeErr != nil { mwErr = append(mwErr, w.TeeErr) }
            mwErr = append(mwErr, &errBuf)
            errW = io.MultiWriter(mwErr...)
        } else {
            if w.TeeOut != nil { outW = io.MultiWriter(outW, w.TeeOut) }
            if w.TeeErr != nil { errW = io.MultiWriter(errW, w.TeeErr) }
        }
        cmd.Stdout = outW
        cmd.Stderr = errW
        cmd.Stdin = ctx.Stdin()
        ...
        if w.CaptureAlso {
            ...
            if ee := toExitError(runErr); ee != nil { res.ExitCode = ee.Code; ctx.Set("__wrapper_result__", res); return ee }
            ctx.Set("__wrapper_result__", res)
        }
        if runErr != nil { return toExitError(runErr) }
        return nil
    case modeCapture:
        var outBuf, errBuf bytes.Buffer
        cmd.Stdout = &outBuf
...
func splitLeading(args []string, leadingSet []string) (leading, rest []string) {
    if len(leadingSet) == 0 { return nil, args }
    isLead := func(s string) bool {
        for _, f := range leadingSet { if s == f { return true } }
        return false
    }
    i := 0
    for ; i < len(args); i++ {
